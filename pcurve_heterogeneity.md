---
title: P-curve is biased by more representative significant effects (which might also
  be called heterogeneity)
...
> "the distribution of statistically significant *p*-values from a set of studies."
...
The tests and the effect size estimation technique for *p*-curve are quite popular, at least popular enough to attract some pretty harsh criticisms. In this post, I focus on one:

> *p*-curve gives biased estimates (i.e., estimates that are too big or too small) when significant *p*-values stem from a heterogeneous distribution of true effect sizes (i.e., the true effect size varies around the average effect size).
...
![**McShane et al. Figure 3**](/Users/nicholasmichalak/Desktop/novum_R_ganum/mcshane_et_al_F3.png) 
...
In fact, between the main text and their supplement, they reported 4 simulations under which *p*-curve recovered accurate average effect sizes even when the true average effect size varied (see Figure 2, panel B in their effect size paper and Figure S2 from their Supplement, [.html](http://p-curve.com/Supplement/Supplement_pcurve2.pdf)).

![**Simonsohn et al. Figure 2**](/Users/nicholasmichalak/Desktop/novum_R_ganum/simonsohn_et_al_F2.png) 


![](/Users/nicholasmichalak/Desktop/novum_R_ganum/simonsohn_et_al_S2.png) 
...
![](pcurve_heterogeneity_files/figure-html/unnamed-chunk-13-1.png)<!-- -->
...
## draw another 5000 "p"-values at random (for each, why not?)
...
![](pcurve_heterogeneity_files/figure-html/unnamed-chunk-23-1.png)<!-- -->
...
> the bias estimate is 4 - 0.53 = -0.13. This bias should look familiar: compare this to the average bias estimate for *p*-curve Figure 3 in McShane et al. I used mean *d* = 0.40, which gives a bias estimate practically in the middle of those for mean *d* = 0.30 (~ 0.15) and mean *d* = 0.50 (~ 0.075) from McShane et al.
...
![](pcurve_heterogeneity_files/figure-html/unnamed-chunk-39-1.png)<!-- -->
...
McShane et al. and even Simonsohn et al. point this out as a major limitation of *p*-curve. McShane et al. go further and recommend that you use the information in those nonsignificant effects. For example, you could use `weightfunct()` from the weightr package to estimate the Iyengar and Greenhouse (1984) model. See below.
...
## 
## tau^2 (estimated amount of total heterogeneity): 0.0384 (SE = 0.0013)
## tau (square root of estimated tau^2 value):  0.1961
## 
## Model Results:
## 
##           estimate std.error z-stat      p-val  ci.lb  ci.ub
## Intercept   0.3973  0.003078  129.1 < 2.22e-16 0.3913 0.4034
## 
## Adjusted Model (k = 10000):
## 
## tau^2 (estimated amount of total heterogeneity): 0.0384 (SE = 0.0013)
## tau (square root of estimated tau^2 value):  0.1959
## 
## Model Results:
## 
##               estimate std.error z-stat      p-val  ci.lb ci.ub
## Intercept       0.3963  0.004916  80.61 < 2.22e-16 0.3867 0.406
## 0.025 < p < 1   0.9915  0.033327  29.75 < 2.22e-16 0.9262 1.057
## 
## Likelihood Ratio Test:
## X^2(df = 1) = 0.06452124, p-val = 0.79949
## 
## Number of Effect Sizes per Interval:
## 
##                      Frequency
## p-values <0.025           4168
## 0.025 < p-values < 1      5832
```
...
## 
##           estimate std.error z-stat      p-val  ci.lb  ci.ub
## Intercept   0.6359   0.00343  185.4 < 2.22e-16 0.6292 0.6426
## 
## Adjusted Model (k = 4168):
## 
## tau^2 (estimated amount of total heterogeneity): 0.0223 (SE = 0.0018)
## tau (square root of estimated tau^2 value):  0.1493
## 
## Model Results:
## 
##               estimate std.error z-stat      p-val  ci.lb  ci.ub
## Intercept       0.4643  0.003533  131.4 < 2.22e-16 0.4573 0.4712
## 0.025 < p < 1   0.0100       NaN    NaN         NA    NaN    NaN
## 
## Likelihood Ratio Test:
## X^2(df = 1) = 2876.314, p-val = < 2.22e-16
## 
## Number of Effect Sizes per Interval:
## 
##                      Frequency
